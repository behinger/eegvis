function varargout=vline(x,in1,in2)
...
if length(x)>1  % vector input
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
            hT = text(x+0.01*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'));
        else
            hT = text(x-.05*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'));
        end
    end     
